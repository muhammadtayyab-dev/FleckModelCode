...
def overlay(image, mask, box, clas, color, alpha, resize=None):
    ...
    image_combined = cv2.rectangle(image_overlay, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255, 0, 234),2)
    image_combined = cv2.putText(image_overlay, str(clas), (int(box[2]), int(box[3])), cv2.FONT_HERSHEY_DUPLEX,1, (255, 255, 0), 1)
    # cv2.imwrite("my2.png",image_overlay)
...
def non_max_suppression(
    ...
    cxa = xc[0].tolist()
    # cal = cxa.filter(lambda x: not x, cxa)
    t = time.time()
...
def postprocess(path, preds, img, orig_imgs, proto):
    ...
    for i, pred in enumerate(p):
        ...
        if retina_masks:
            ...
            firstFing = pred[:1, :4][0]
            x1 = firstFing[0]
            y1 = firstFing[1]
            x2 = firstFing[2]
            y2 = firstFing[3]
            # ops.
            masks = process_mask(proto[i], pred[:, 6:], pred[:, :4], img.shape[1:3], upsample=True)  # HWC
            # if not isinstance(orig_imgs, torch.Tensor):
...
model_path = "best_float16_32.tflite"
